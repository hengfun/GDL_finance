...
class MLPEncoder(nn.Module):
    ...
    def edge2node(self, x, rel_rec, rel_send):
        ...
        # rel_rec_ = rel_rec.to_sparse().t()
        bs,stocks,d = x.shape
        x_ = x.transpose(1,0)
        x_ = x_.reshape(stocks,-1)      
        incoming = torch.mm(rel_rec.t(),x_).view(-1,bs,d) 
        incoming = incoming.transpose(0,1)
    ...
    def node2edge(self, x, rel_rec, rel_send):
        ...
        # rel_rec_ = rel_rec.to_sparse()
        # rel_send_ = rel_send.to_sparse()
        bs,stocks,d = x.shape
        x_ = x.transpose(1,0)
        x_ = x_.reshape(stocks,-1)        
        receivers = torch.mm(rel_rec,x_).reshape(-1,bs,d)
        receivers = receivers.transpose(1,0)
        senders = torch.mm(rel_send,x_).reshape(-1,bs,d)
        senders = senders.transpose(1,0)
...
class MLPDecoderP(nn.Module):
    """MLP decoder module."""

    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
                 do_prob=0., skip_first=False):
        super(MLPDecoderP, self).__init__()
        self.msg_fc1 = nn.ModuleList(
            [nn.Linear(2 * n_in_node, msg_hid) for _ in range(edge_types)])
        self.msg_fc2 = nn.ModuleList(
            [nn.Linear(msg_hid, msg_out) for _ in range(edge_types)])
    ...
    def single_step_forward(self, single_timestep_inputs, rel_rec, rel_send,
        ...
        # Node2edge
        batch_size, num_timesteps, num_atoms, num_dims = single_timestep_inputs.shape
        batch_size, num_timesteps, num_atoms2, num_edge_types = single_timestep_rel_type.shape

        single_timestep_inputs_ = single_timestep_inputs.contiguous().transpose(0,2).contiguous().view(num_atoms,-1)
        receivers = torch.mm(rel_rec,single_timestep_inputs_)
        receivers = receivers.reshape(-1,batch_size,num_timesteps,num_dims).transpose(0,1).transpose(1,2)

        # receivers = torch.matmul(rel_rec, single_timestep_inputs)
        
        # senders = torch.matmul(rel_send, single_timestep_inputs)
        senders = torch.mm(rel_send,single_timestep_inputs_)
        senders = senders.reshape(-1,batch_size,num_timesteps,num_dims).transpose(0,1).transpose(1,2)
        ...
        for i in range(start_idx, len(self.msg_fc2)):
            ...
            msg = msg * single_timestep_rel_type[:, :, :, i:i + 1]
            all_msgs += msg
        i,j,k,l = all_msgs.shape
        all_msgs_ = all_msgs.contiguous().transpose(0,2).contiguous().view(rel_rec.size(0),-1)
        agg_msgs= torch.mm(rel_rec.t(),all_msgs_).reshape(num_atoms,j,i,l).transpose(0,2)

        # Aggregate all msgs to receiver
        # agg_msgs = all_msgs.transpose(-2, -1).matmul(rel_rec.to_dense()).transpose(-2, -1)
        # agg_msgs = agg_msgs.contiguous()
...
class RNNDecoderP(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, n_hid,
                 do_prob=0., skip_first=False,n_out=1):
        super(RNNDecoderP, self).__init__()
        self.msg_fc1 = nn.ModuleList(
            [nn.Linear(2 * n_hid, n_hid) for _ in range(edge_types)])
        self.msg_fc2 = nn.ModuleList(
            [nn.Linear(n_hid, n_hid) for _ in range(edge_types)])
        ...
        self.out_fc2 = nn.Linear(n_hid, n_hid)
        self.out_fc3 = nn.Linear(n_hid, n_in_node)
        # self.out_fc3 = nn.Linear(n_hid, n_out)
    ...
    def single_step_forward(self, inputs, rel_rec, rel_send,
        ...
        # node2edge
        hidden_ = hidden.transpose(0,1)
        stocks, bs,d = hidden_.shape 
        hidden_ = hidden_.reshape(stocks,-1)
        receivers_ = torch.mm(rel_rec,hidden_).reshape(-1,bs,d)
        receivers_ = receivers_.transpose(0,1)

        senders_ = torch.mm(rel_send,hidden_).reshape(-1,bs,d)
        senders_ = senders_.transpose(0,1)
        pre_msg = torch.cat([receivers_, senders_], dim=-1)
        ...
        all_msgs_ =all_msgs.transpose(0, 1).contiguous().view(-1,bs*d)
        agg_msgs = torch.mm(rel_rec.transpose(0,1),all_msgs_)
        # agg_msgs.view(-1,bs,d)
        agg_msgs = agg_msgs.view(-1,bs,d).transpose(0,1)
        # agg_msgs = all_msgs.transpose(-2, -1).matmul(rel_rec.to_dense()).transpose(-2,
        #                                                                 -1)
        agg_msgs = agg_msgs.contiguous() / inputs.size(2)  # Average
