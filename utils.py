...
def load_data(batch_size=1, suffix='',num_workers=1):
    ...
    train_data_loader = DataLoader(train_data, batch_size=batch_size,num_workers=num_workers)
    valid_data_loader = DataLoader(valid_data, batch_size=batch_size,num_workers=num_workers)
...
def rescale(x):
    ...
    x_tmp = x.reshape(-1,l)
    t = MaxAbsScaler().fit(x_tmp)
    x_tmp = t.transform(x_tmp)
    x = x_tmp.reshape(i,j,k,l)
    return x
        
def load_finance_data(batch_size=1, suffix='',num_workers=1,index=-1,stocks=50,
                      train_idx=120,val_idx=120,test_len=100):
    # [num_samples, num_timesteps, num_dims, num_atoms]
    loc_max = 1
    loc_min = -1
    vel_max = 1
    vel_min = -1
    x = np.load('x_fixed.npy')
    y = np.load('y_fixed.npy')
    ...
    x = x.reshape(i,j,-1,train_idx+val_idx,l)
    x = np.transpose(x,[0,2,1,3,4])
    x = x.reshape(-1,j,train_idx+val_idx,l)
    ...
    i,j,k = y.shape
    y_tmp = y.reshape(-1,k)
    t = MaxAbsScaler().fit(y_tmp)
    y_tmp = t.transform(y_tmp)
    y = y_tmp.reshape(i,j,k)
    y = y.reshape(-1,val_idx+train_idx,k)
    
    xtrain = x[:,:stocks,:train_idx,:]
    xval = x[:,:stocks,train_idx:train_idx+val_idx,:]
    
    ytrain = y[:,:train_idx,index]
    yval = y[:,train_idx:train_idx+val_idx,index]

    xtrain2 = xtrain[-test_len:,:,:,:]
    ytrain2 = ytrain[-test_len:]
    xval2 = xval[-test_len:,:,:,:]
    yval2 = yval[-test_len:]
    xtest = np.concatenate([xtrain2,xval2],axis=0)
    ytest = np.concatenate([ytrain2,yval2],axis=0)

    xtrain = torch.FloatTensor(xtrain[:-test_len,:,:,:])
    ytrain = torch.FloatTensor(ytrain[:-test_len,:])
    xval = torch.FloatTensor(xval[:-test_len,:,:,:])
    yval = torch.FloatTensor(yval[:-test_len,:])
    ...
    valid_data = TensorDataset(xval,yval)
    test_data = TensorDataset(xtest,ytest)

    train_data_loader = DataLoader(train_data, batch_size=batch_size,num_workers=num_workers,shuffle=False,drop_last=True)
    valid_data_loader = DataLoader(valid_data, batch_size=batch_size,num_workers=num_workers,shuffle=False,drop_last=True)
    test_data_loader = DataLoader(test_data, batch_size=batch_size,num_workers=num_workers,shuffle=False,drop_last=True)

    return train_data_loader, valid_data_loader, test_data_loader, loc_max, loc_min, vel_max, vel_min


def load_kuramoto_data(batch_size=1, suffix='',num_workers=1):
...
def load_kuramoto_data_old(batch_size=1, suffix='',num_workers=1):
...
def load_motion_data(batch_size=1, suffix='',num_workers=1):
...
def edge_accuracy2(preds, target):
    i,j,k = preds.shape
    _, target = preds.max(-1)
    target = torch.ones((i,j)).cuda()
    _, preds = preds.max(-1)
    correct = preds.float().data.eq(
        target.float().data.view_as(preds)).cpu().sum()
    return np.float(correct) / (target.size(0) * target.size(1))
